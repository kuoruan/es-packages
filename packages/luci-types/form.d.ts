...
declare namespace form {
  ...
  class JSONMap extends Map {
     ...
     *
     * @param data - The JavaScript object to use as data source. Internally, the object is converted into an UCI-like format. Its toplevel keys are treated like UCI section types while the object or array-of-object values are treated as section contents.
     * @param title - The title caption of the form. A form title is usually rendered as separate headline element before the actual form contents. If omitted, the corresponding headline element will not be rendered.
     * @param description - The description text of the form which is usually rendered as text paragraph below the form title and before the actual form conents. If omitted, the corresponding paragraph element will not be rendered.
     */
    constructor(
      data: { [key: string]: any },
      title?: string,
  ...
  class Map extends AbstractElement {
     ...
     *
     * @param config - The UCI configuration to map. It is automatically loaded along when the resulting map instance.
     * @param title - The title caption of the form. A form title is usually rendered as separate headline element before the actual form contents. If omitted, the corresponding headline element will not be rendered.
     * @param description - The description text of the form which is usually rendered as text paragraph below the form title and before the actual form conents. If omitted, the corresponding paragraph element will not be rendered.
     */
     ...
     *
     * @param config - The additional UCI configuration file to tie to the map. If the given config already is in the list of required files, it will be ignored.
     */
     ...
     *
     * @param selector_or_attrname - If invoked with only one parameter, this argument is a `querySelector()` compatible selector expression. If invoked with two parameters, this argument is the attribute name to filter for.
     * @param attrvalue - In case the function is invoked with two parameters, this argument specifies the attribute value to match.
     *
     * @throws Throws an {@link InternalError} if more than two function parameters are passed.
     *
     ...
     *
     * @param selector_or_attrname - If invoked with only one parameter, this argument is a `querySelectorAll()` compatible selector expression. If invoked with two parameters, this argument is the attribute name to filter for.
     * @param attrvalue - In case the function is invoked with two parameters, this argument specifies the attribute value to match.
     *
     * @throws Throws an {@link InternalError} if more than two function parameters are passed.
     *
     * @returns Returns a (possibly empty) DOM `NodeList` containing the found DOM nodes.
     */
     ...
     *
     * @returns Returns a promise resolving once the entire form completed loading all data. The promise may reject with an error if any configuration failed to load or if any of the child elements load functions rejected with an error.
     */
    load(): Promise<void>;
     ...
     *
     * @param name_or_id - The name or the full ID of the option element to look up.
     * @param section_id - The ID of the UCI section containing the option to look up. May be omitted if a full ID is passed as first argument.
     * @param config - The name of the UCI configuration the option instance is belonging to. Defaults to the main UCI configuration of the map if omitted.
     *
     * @returns Returns a two-element array containing the form option instance as first item and the corresponding UCI section ID as second item. Returns `null` if the option could not be found.
     */
     ...
     * Elements which are hidden due to unsatisified dependencies are skipped.
     *
     * @returns Returns a promise resolving once the entire form completed parsing all input values. The returned promise is rejected if any parsed values are not meeting the validation constraints of their respective elements.
     */
    parse(): Promise<void>;
     ...
     *
     * @returns Returns a promise resolving to the toplevel form DOM node once the rendering is complete.
     */
    render(): Promise<Node>;
     ...
     *
     * @returns Returns a promise resolving to the toplevel form DOM node once the re-rendering is complete.
     */
    reset(): Promise<Node>;
     ...
     *
     * @param cb - An optional callback function that is invoked after the form is parsed but before the changed UCI data is saved. This is useful to perform additional data manipulation steps before saving the changes.
     * @param silent - If set to `true`, trigger an alert message to the user in case saving the form data failes. Otherwise fail silently.
     *
     * @returns Returns a promise resolving once the entire save operation is complete. The returned promise is rejected if any step of the save operation failed.
     */
     ...
     *
     * @param sectionclass - The section class to use for rendering the configuration section. Note that this value must be the class itself, not a class instance obtained from calling `new`. It must also be a class dervied from `LuCI.form.AbstractSection`.
     * @param classargs - repeatable. Additional arguments which are passed as-is to the contructor of the given section class. Refer to the class specific constructor documentation for details.
     *
  ...
  class NamedSection extends AbstractSection {
     ...
     *
     * @param form - The configuration form this section is added to. It is automatically passed by `section()`.
  ...
  > extends Value<string, S> {
     ...
     *
     * @param form - The configuration form this section is added to. It is automatically passed by `option()` or `taboption()` when adding the option to the section.
     * @param section - The configuration section this option is added to. It is automatically passed by `option()` or `taboption()` when adding the option to the section.
     * @param option - The internal name of the option element holding the section. Since a section container element does not read or write any configuration itself, the name is only used internally and does not need to relate to any underlying UCI option name.
     * @param subsection_class - The class to use for instantiating the nested section element. Note that the class value itself is expected here, not a class instance obtained by calling `new`. The given class argument must be a subclass of the `AbstractSection` class.
     * @param class_args - repeatable. All further arguments are passed as-is to the subclass constructor. Refer to the corresponding class constructor documentations for details.
     */
    constructor(
      form: Map | JSONMap,
  ...
  class TableSection extends TypedSection {
    ...
    /**
     * Override the per-section instance modal popup title caption shown when clicking the `More…` button in a section specifying `max_cols`.
     *
     * If set to a string, it will be used as `String.format()` pattern with the name of the underlying UCI section as first argument, if set to a function, the function will be invoked with the section name as first argument and its return value is used as caption, after converting it to a string.
     *
     * If this property is not set, the default is the name of the underlying UCI configuration section.
     *
     * @defaultValue null
     */
    ...
    /**
     * Override the per-section instance title caption shown in the first column of the table unless `anonymous` is set to true.
     *
     * If set to a string, it will be used as `String.format()` pattern with the name of the underlying UCI section as first argument, if set to a function, the function will be invoked with the section name as first argument and its return value is used as caption, after converting it to a string.
     *
     * If this property is not set, the default is the name of the underlying UCI configuration section.
     ...
     *
     * @param modalSection - The `NamedSection` instance about to be rendered in the modal popup.
     * @param section_id - The ID of the underlying UCI section the modal popup belongs to.
  ...
  class TypedSection extends AbstractSection {
    /**
     * Override the caption used for the section add button at the bottom of the section form element. If set to a string, it will be used as-is, if set to a function, the function will be invoked and its return value is used as caption, after converting it to a string.
     *
    ...
    /**
     * Override the UCI configuration name to read the section IDs from. By default, the configuration name is inherited from the parent `Map`. By setting this property, a deviating configuration may be specified.
     *
     * The default is `null`, means inheriting from the parent form.
     *
     * @defaultValue null
  ...
  > extends AbstractValue<T> {
     ...
     * @param value - The caption for the choice value. May be a DOM node, a document fragment or a plain text string. If omitted, the `key` value is used as caption.
     */
