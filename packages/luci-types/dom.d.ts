// Type definitions for LuCI.dom
// Documentation: http://openwrt.github.io/luci/jsapi/LuCI.dom.html
...
declare namespace dom {
   ...
   * @param attr - Specifies an Object of key, value pairs to set as attributes or event handlers on the created node. Refer to `dom.attr()` for details.
   * @param data - Specifies children to append to the newly created element. Refer to `dom.append()` for details.
   *
   * @throws Throws an {@link InvalidCharacterError} when the given `html` argument contained malformed markup (such as not escaped `&` characters in XHTML mode) or when the given node name in `html` contains characters which are not legal in DOM element names, such as spaces.
   *
   ...
   * @param key - This is either a string specifying the key to retrieve, or `null` to unset the entire node data.
   * @param val - This is either a non-`null` value to set for a given key or `null` to remove the given `key` from the specified node.
   *
   * @returns Returns the get or set value, or `null` when no value could be found.
   */
  function data<T = string>(
    node: Node,
    key?: string | null,
    val?: any
  ): T | null;
   ...
   * @param node - The DOM `Node` instance to start from.
   *
   * @returns Returns the founds class instance if any or `null` if no bound class could be found on the node itself or any of its parents.
   */
   ...
   * @param ignoreFn - Specifies an optional function which is invoked for each child node to decide whether the child node should be ignored or not.
   *
   * @returns Returns `true` if the node does not have any children or if any children node either has a `hidden` CSS class or a `false` result when testing it using the given `ignoreFn`.
   */
   ...
   * @param selector - The query selector expression to test against the given node.
   *
   * @returns Returns `true` if the given node matches the specified selector or `false` when the node argument is no valid DOM `Node` or the selector didn't match.
   */
   ...
   * @param selector - The query selector expression to test against each parent.
   *
   * @returns Returns the closest parent node matching the selector or `null` when the node argument is no valid DOM `Node` or the selector didn't match any parent.
   */
  function parent(node: Node, selector?: string): Node | null;

  /**
   * Parses a given string as HTML and returns the first child node.
   *
   * @param s - A string containing an HTML fragment to parse. Note that only the first result of the resulting structure is returned, so an input value of `<div>foo</div> <div>bar</div>` will only return the first `div` element node.
   *
   * @returns Returns the first DOM `Node` extracted from the HTML fragment or `null` on parsing failures or if no element could be found.
   */
