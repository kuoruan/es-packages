// Type definitions for LuCI.uci
// Documentation: http://openwrt.github.io/luci/jsapi/LuCI.uci.html
...
declare namespace uci {
   ...
   * @param name - The name of the section to add. If the name is omitted, an anonymous section will be added instead.
   *
   * @returns Returns the section ID of the newly added section which is equivalent to the given name for non-anonymous sections.
   */
   ...
   *
   * @param timeout - Override the confirmation timeout after which a rollback is triggered.
   *
   * @returns Returns a promise resolving/rejecting with the `ubus` RPC status code.
   */
   ...
   *
   * @returns Returns a promise resolving to an object containing the configuration names as keys and arrays of related change records as values.
   */
  function changes(): Promise<{ [key: string]: ChangeRecord[] }>;
   ...
   *
   * @returns A newly generated, unique section ID in the form `newXXXXXX` where `X` denotes a hexadecimal digit.
   */
  function createSID(config: string): string;
   ...
   * @param option - The option name to read the value from. If the option name is omitted or `null`, the entire section is returned instead.
   *
   * @returns
   ...
   *
   * @param config - The name of the configuration to read the value from.
   * @param type - The type of the first section to find. If it is null, the first section of the entire config is read, otherwise the first section matching the given type.
   * @param option - The option name to read the value from. If the option name is omitted or null, the entire section is returned instead.
   *
   * @returns
   * - Returns a string containing the option value in case of a plain UCI option.
  ...
  function get_first(config: string, type?: string): SectionObject | null;
   ...
   *
   * @param config - The name of the configuration or an array of configuration names to load.
   *
   * @returns Returns a promise resolving to the names of the configurations that have been successfully loaded.
   */
   ...
   * @param sid2 - The ID of the target section for the move operation. If the `after` argument is `false` or not specified, the section named by `sid1` will be moved before this target section, if the `after` argument is `true`, the `sid1` section will be moved after this section.
   *
   * When the `sid2` argument is `null`, the section specified by `sid1` is moved to the end of the configuration.
   * @param after - When `true`, the section `sid1` is moved after the section `sid2`, when `false`, the section `sid1` is moved before `sid2`.
   *
   * If `sid2` is null, then this parameter has no effect and the section `sid1` is moved to the end of the configuration instead.
   *
   * @returns Returns `true` when the section was successfully moved, or `false` when either the section specified by `sid1` or by `sid2` is not found.
   */
  function move(
    config: string,
    sid1: string,
   ...
   */
  function remove(config: string, sid: string): void;
   ...
   * @param sid - The section ID to resolve. If the ID is in the form `@typename[#]`, it will get resolved to an internal anonymous ID in the forms `cfgXXXXXX`/`newXXXXXX` or to the name of a section in case it points to a named section. When the given ID is not in extended notation, it will be returned as-is.
   *
   * @returns Returns the resolved section ID or the original given ID if it was not in extended notation. Returns `null` when an extended ID could not be resolved to existing section ID.
   */
  function resolveSID(config: string, sid: string): string | null;
   ...
   *
   * @returns Returns a promise resolving to an array of configuration names which have been reloaded by the save operation.
   */
   ...
   *
   * @param config - The name of the configuration to enumerate the sections for.
   * @param type - Enumerate only sections of the given type. If omitted, enumerate all sections.
   * @param cb - An optional callback to invoke for each enumerated section.
   *
   * @returns Returns a sorted array of the section objects within the given configuration, filtered by type of a type has been specified.
   */
  function sections(
    config: string,
    type?: string,
   ...
   * @param value - The option value to set. If the value is `null` or an empty string, the option will be removed, otherwise it will be set or overwritten with the given value.
   */
  function set(
    config: string,
    sid: string,
   ...
   *
   * @param config - The name of the configuration to set the option value in.
   * @param type - The type of the first section to find. If it is `null`, the first section of the entire config is written to, otherwise the first section matching the given type is used.
   * @param option - The option name to set the value for.
   * @param value - The option value to set. If the value is `null` or an empty string, the option will be removed, otherwise it will be set or overwritten with the given value.
   */
  function set_first(
   ...
   *
   * @param config - The name of the configuration or an array of configuration names to unload.
   */
  function unload(config: string | string[]): void;
   ...
   */
  function unset(config: string, sid: string, option: string): void;
  ...
  type ChangeRecord = {
     ...
     */
    0: "add" | "set" | "remove" | "order" | "list-add" | "list-del" | "rename";
