declare namespace LuCI {
  class network extends LuCI.baseclass {
    /**
     ...
     */
    getDevices(): Promise<LuCI.network.Device[]>;
     ...
     */
    getNetwork(name: string): Promise<LuCI.network.Protocol | null>;
     ...
     */
    getNetworks(): Promise<LuCI.network.Protocol[]>;
     ...
     */
    getProtocols(): LuCI.network.Protocol[];
     ...
     * layout.
     */
    getSwitchTopologies(): Promise<Record<string, LuCI.network.SwitchTopology>>;
     ...
     */
    getWAN6Networks(): Promise<LuCI.network.Protocol[]>;
     ...
     */
    getWANNetworks(): Promise<LuCI.network.Protocol[]>;
     ...
     * could not be found.
     */
    getWifiDevice(devname: string): Promise<LuCI.network.WifiDevice>;
     ...
     */
    getWifiDevices(): Promise<LuCI.network.WifiDevice[]>;
     ...
     * could not be found.
     */
    getWifiNetwork(netname: string): Promise<LuCI.network.WifiNetwork | null>;
     ...
     * are found.
     */
    getWifiNetworks(): Promise<LuCI.network.WifiNetwork[]>;
  ...
  namespace network {
     ...
     */
    class Device extends LuCI.baseclass {
      /**
       * Get the bridge ID
       *
     ...
     */
    class Hosts extends LuCI.baseclass {
      /**
     ...
     */
    abstract class Protocol extends LuCI.baseclass {
      /**
     ...
     */
    class WifiDevice extends LuCI.baseclass {
      /**
     ...
     */
    class WifiNetwork extends LuCI.baseclass {
      /**
      ...
      /**
       * Get the names of the logical interfaces this wireless network is attached
       * to.
       *
       ...
       *
       * @returns Returns an array of `Network.Protocol` instances representing
       * the logical interfaces this wireless network is attached to.
       */
      getNetworks(): Protocol[];
       ...
       *
       * @returns Returns the radio noise floor in dBm as reported by `ubus`
       * runtime information or `0` if it cannot be determined.
       */
      getNoise(): number;
       ...
       *
       * @returns Returns a string describing this network, consisting of the
       * active operation mode, followed by either the SSID, BSSID or internal
       * network ID, depending on which information is available.
       */
      getShortName(): string;
       ...
       *
       * @returns Returns the wireless signal in dBm as reported by `ubus` runtime
       * information or `null` if it cannot be determined.
       */
       ...
       *
       * @returns Returns the calculated signal level, which is the difference
       * between noise and signal (SNR), divided by 5.
       *
       * @deprecated Do not use.
       */
       ...
       *
       * @returns   Returns the calculated signal quality in percent. The value is
       * calculated from the `quality` and `quality_max` indicators reported by
       * `ubus` runtime state.
       */
       ...
       *
       * @returns Returns the configured SSID value or `null` when this network is
       * in mesh mode.
       */
      getSSID(): string | null;
       ...
       *
       * @returns Returns the wireless network transmit power in dBm as reported
       * by `ubus` runtime information or `null` if it cannot be determined.
       */
       ...
       *
       * Some wireless radios have a fixed power offset, e.g. due to the use of
       * external amplifiers.
       *
       * @returns Returns the wireless network transmit power offset in dBm as
       * reported by `ubus` runtime information or `0` if there is no offset, or
       ...
       *
       * @returns Returns a `Network.WifiDevice` instance representing the
       * corresponding wifi radio device or `null` if the related radio device
       * could not be found.
       */
       ...
       *
       * @returns Returns the name of the radio device this network is configured
       * on or `null` if it cannot be determined.
       */
       ...
       *
       * @returns Returns `true` when this wifi network instance supports forcibly
       ...
       *
       * This function actually queries the up state of the related radio device
       * and assumes this network to be up as well when the parent radio is up.
       * This is due to the fact that OpenWrt does not control virtual interfaces
       * individually but within one common hostapd instance.
       *
    ...
    /**
     * Describes an swconfig switch topology by specifying the CPU connections and
     * external port labels of a switch.
     */
    type SwitchTopology = {
      /**
       * The `netdevs` property points to an object describing the CPU port
       * connections of the switch. The numeric key of the enclosed object is the
       * port number, the value contains the Linux network device name the port is
       * hardwired to.
       */
      netdevs: Record<number, string>;

      /**
       * The `ports` property points to an array describing the populated ports of
       * the switch in the external label order. Each array item is an object
       ...
       */
      ports: Record<string, boolean | number | string>[];
    };

    /**
     * An encryption entry describes active wireless encryption settings such as
     * the used key management protocols, active ciphers and protocol versions.
     */
    type WifiEncryption = {
      /**
      ...
      /**
       * When the `wep` property exists, the network uses WEP encryption. In this
       * case, the property is set to an array of active WEP modes which might be
       * either `open`, `shared` or both.
       */
      wep?: string[];

      /**
       * When the `wpa` property exists, the network uses WPA security. In this
       * case, the property is set to an array containing the WPA protocol
       * versions used, e.g. `[ 1, 2 ]` for WPA/WPA2 mixed mode or `[ 3 ]` for
       * WPA3-SAE.
       */
      wpa?: number[];

      /**
       * The `authentication` property only applies to WPA encryption and is
       * defined when the `wpa` property is set as well. It points to an array of
       * active authentication suites used by the network, e.g. `[ "psk" ]` for a
       * WPA(2)-PSK network or `[ "psk", "sae" ]` for mixed WPA2-PSK/WPA3-SAE
       * encryption.
       */
      authentication?: string[];

      /**
       * If either WEP or WPA encryption is active, then the `ciphers` property
       * will be set to an array describing the active encryption ciphers used by
       * the network, e.g. `[ "tkip", "ccmp" ]` for a WPA/WPA2-PSK mixed network
       * or `[ "wep-40", "wep-104" ]` for an WEP network.
       */
      ciphers?: string[];
    };

    /**
     * A wireless peer entry describes the properties of a remote wireless peer
     * associated with a local network.
     */
    type WifiPeerEntry = {
      /**
      ...
      /**
       * The amount of milliseconds the peer has been inactive, e.g. due to
       * powersave.
       */
      inactive: number;
      ...
      /**
       * The mesh LLID, may be `0` or absent if not applicable or supported by the
       * driver.
       */
      "mesh llid"?: number;

      /**
       * The mesh PLID, may be `0` or absent if not applicable or supported by the
       * driver.
       */
      "mesh plid"?: number;

      /**
       * The mesh peer link state description, may be an empty string (`''`) or
       * absent if not applicable or supported by the driver.
       *
      ...
      /**
       * The local powersafe mode for the peer link, may be an empty string (`''`)
      ...
      /**
       * The remote powersafe mode for the peer link, may be an empty string
       * (`''`) or absent if not applicable or supported by the driver.
       *
       * The following modes are known:
       * - `ACTIVE` (no power save)
      ...
      /**
       * The powersafe mode for all non-peer neigbours, may be an empty string
       * (`''`) or absent if not applicable or supported by the driver.
       *
       * The following modes are known:
       * - `ACTIVE` (no power save)
    ...
    /**
     * A wireless rate entry describes the properties of a wireless transmission
     * rate to or from a peer.
     */
    type WifiRateEntry = {
      /**
       * The amount of received misc. packages that have been dropped, e.g. due to
       * corruption or missing authentication. Only applicable to receiving rates.
       */
      drop_misc?: number;
      ...
      /**
       * The amount of failed tranmission attempts. Only applicable to transmit
       * rates.
       */
      failed?: number;
      ...
      /**
       * The MCS index of the used transmission rate. Only applicable to HT or VHT
       * rates.
       */
      mcs?: number;

      /**
       * Specifies whether the tranmission rate used 40MHz wide channel. Only
       * applicable to HT or VHT rates.
       *
       * Note: this option exists for backwards compatibility only and its use is
       * discouraged. The `mhz` field should be used instead to determine the
       * channel width.
       */
      "40mhz"?: number;
      ...
      /**
       * Specifies the number of spatial streams used by the transmission. Only
       * applicable to VHT rates.
       */
      nss?: number;
    };

    /**
     * A wireless scan result object describes a neighbouring wireless network
     * found in the vincinity.
     */
    type WifiScanResult = {
      /**
      ...
      /**
       * The numeric quality level of the signal, can be used in conjunction with
       * `quality_max` to calculate a quality percentage.
       */
      quality: number;
